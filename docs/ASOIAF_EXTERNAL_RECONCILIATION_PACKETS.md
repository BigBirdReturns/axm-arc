...
The focused qualification proves deterministic packet identity, private filename non-disclosure, exact evidence-bundle construction, a passing holder-controlled primary-text confirmation, refusal of intake-only observations, reviewer-crossing refusal, community-primary refusal, analogue-promotion refusal, and continuity isolation. The permanent workflow also runs strict TypeScript, the complete Arc regression suite, the production build, and a deterministic sample receipt.
