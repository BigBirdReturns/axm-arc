...
const PRE_ACTION_CANDIDATES: NarrativeCandidate[] = [
  ...
  candidate({
    id: "underdrain-aquifer-reveal",
    recipeId: "villainy-becomes-legible-pressure",
    track: { kind: "advance", trackId: "underdrain-war" },
    beatFunction: "reveal",
    sourceFactIds: ["fact-aquifer-poisoning"],
    causalParentBeatIds: [],
    roleBindings: { envoy: "morrowcap", plumber: "rhea-venn" },
    actorMoves: [{ actorId: "morrowcap", moveTag: "negotiate" }],
    tags: ["institution:municipal", "ordinary:plumbing", "truth:shared-harm"],
    pressureTags: ["pressure:ecology"],
    statePayments: [{ kind: "precedent", target: "fungus-cause-is-legible", tags: ["payment:precedent"] }],
    opensObligations: [{
      id: "expose-enzyme-poisoning",
      kind: "evidence-duty",
      actorIds: ["rhea-venn", "marta-sump"],
      tags: ["truth:shared-harm", "pressure:ecology"],
      pressure: 8,
      dueCycle: 2,
    }],
    resolvesObligationIds: [],
    ...
    presentationKey: "underdrain.aquifer-reveal",
  }),
];
...
const STRATEGY_CANDIDATES: Record<UnderdrainStrategyId, NarrativeCandidate> = {
  "emergency-plan": candidate({
    ...
    track: { kind: "advance", trackId: "underdrain-war" },
    beatFunction: "choose",
    sourceFactIds: ["fact-draft-order", "fact-aquifer-poisoning"],
    causalParentBeatIds: [],
    roleBindings: { plumber: "rhea-venn" },
