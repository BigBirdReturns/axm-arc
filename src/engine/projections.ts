import type { Agent, Arc, Challenge, MechanicCheck, Organization } from "./types.js";
  ...
  for (const check of challenge.mechanicChecks) {
    if (check.scope === "team_aggregate") {
      const teamScore = assignedAgents.reduce(
        (s, a) => s + estimateScore(a, check, assignedAgents, org, arc), 0,
      );
      ...
      for (const a of assignedAgents) {
        ...
        if (m < worstMargin) { worstMargin = m; worstAgent = a; }
      }
...
function estimateScore(
  ...
  const rawScore = check.attributeWeights.reduce(
    (s, aw) => s + (agent.attributes[aw.attributeId] ?? 0) * aw.weight, 0,
  );
  ...
  const afflictionMod = agent.afflictionState.kind !== "none"
  ...
  for (const tid of agent.traits) {
    ...
    for (const fx of t.effects) {
      if (fx.kind === "attributeCheckBonus" && check.attributeWeights.some((aw) => aw.attributeId === fx.attributeId)) {
        traitBonus += fx.bonus;
      }
    }
  }
...
export function predictImminentEvents(org: Organization, arc: Arc): string[] {
  ...
  for (const [, agent] of Object.entries(org.agents)) {
    if (agent.stress >= 8 && agent.afflictionState.kind === "none") {
      events.push(`${agent.name} affliction in ~${10 - agent.stress} cycle${10 - agent.stress === 1 ? "" : "s"}`);
    }
    ...
    for (const t of traitThresholds) {
      if (assignments < t && assignments >= t - 3) {
        events.push(`${agent.name} trait reveal in ${t - assignments} job${t - assignments === 1 ? "" : "s"}`);
        break;
      }
    }
  }
