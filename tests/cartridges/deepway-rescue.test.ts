// Deepway Rescue completion-sim acceptance: exercises the reusable
// cartridge-conformance harness (src/sim/cartridge-conformance.ts) against
// `cartridges/deepway-rescue.arc.json` — a new importable cartridge, not yet
...
// honesty, reachability/tuning) but through the arc-agnostic helper instead
// of a cartridge-specific simulator.
...
// Pinned content-identity anchor for cartridges/deepway-rescue.arc.json. If
// the JSON is legitimately retuned (see the reachability suite below), this
// must be recomputed and updated — loudly, in the same change — never
// silently.
const PINNED_DIGEST = "cart1_50ac135555a90ca793ca3a04e100494c2836a680e66ca7a8d494c479c3dae00a";

const CARTRIDGE_PATH = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)),
  "../../cartridges/deepway-rescue.arc.json",
);
...
function loadArc(): Arc {
  ...
  if (!imported.ok) throw new Error(`deepway-rescue.arc.json failed to import: ${imported.errors.join("; ")}`);
...
describe("import", () => {
  it("imports through the real seam (importArcFromJson → validateArc) without errors", () => {
    ...
    if (!result.ok) return;
    expect(result.entry.arc.meta.id).toBe("deepway-rescue");
...
const CHAPTER_2_CHALLENGES = ["collapsed-stair", "gas-pocket", "breakaway-apprentice"];
const CHAPTER_3_CHALLENGES = ["the-deep-beacon", "hollow-crown"];
...
describe("gate honesty", () => {
  ...
  it("hollow-crown (the finale) is only ever attempted once the-deep-beacon-charts is held by an attuned agent", { timeout: LONG }, () => {
    const arc = loadArc();
    for (const seed of [1, 2, 3, 4, 5, 6, 7, 8]) {
      const run = simulateArcRun(arc, { seed, maxCycles: 45 });
      if (run.attempts["hollow-crown"] !== undefined) {
        // The gate is attunementThreshold 0.5 over agentAttunements
        // ["the-deep-beacon-charts"] (grantsAccessTo on the chain) — an
        // attempt can only exist if challengeAccess judged the actual party
        // as satisfying it, and that requires the chain to have completed
        // for at least one party member.
        expect(run.attunementCycles["the-deep-beacon-charts"]).not.toBeNull();
        // The chain's own steps require the-deep-beacon cleared, which in
...
//
// Reviewer's flagged risk: hollow-crown is difficultyRating 78 with only a
// 3-5 agent party (lampwright + quartermaster required), gated behind the
// the-deep-beacon-charts attunement chain (clear collapsed-stair,
// breakaway-apprentice, and the-deep-beacon — all with the SAME attuned
// agents, since a chain step only completes for an agent that personally
// holds the clear). Verified empirically with this harness before writing
// these assertions: with the harness's generic starting roster (2 agents per
// role, top two tiers) and a bounded seed/cycle sweep, hollow-crown clears in
// the overwhelming majority of seeds within the reachability budget below,
// with ZERO engine-reported failures anywhere in the run — every non-clear
// is a "partial" (agent downtime, not defeat) that the harness's autoplay
// policy simply retries. No content tuning was needed: the encounter is
// tight (partial retries before the final clear are common) but reachable
// and clearable as authored. If a future edit to the JSON changes this
// balance, re-run this suite; a swing to `stalled`/`max-cycles` dominance
// here is the signal that tuning (thresholds, roster ranges, item bonuses —
// smallest change that restores tight-but-reachable) is warranted, exactly
// as docs/balance/KARAZHAN_BALANCE_SIM_REPORT.md frames it for Karazhan.
...
const ALL_CHALLENGE_IDS = [
  ...
  "collapsed-stair",
  "gas-pocket",
  "breakaway-apprentice",
  "the-deep-beacon",
  "hollow-crown",
];

describe("reachability + tuning (hollow-crown finale)", () => {
  it("at least one run clears every chapter-1 and chapter-2 challenge", { timeout: LONG }, () => {
    ...
    for (const id of ["lost-surveyor", "trapped-caravan", "flooded-gallery", "collapsed-stair", "gas-pocket", "breakaway-apprentice"]) {
      expect(runs.some((r) => r.clearCycles[id] !== undefined)).toBe(true);
    }
  });

  it("at least one run completes the the-deep-beacon-charts attunement chain", { timeout: LONG }, () => {
    const arc = loadArc();
    const runs = REACHABILITY_SEEDS.map((seed) => simulateArcRun(arc, { seed, maxCycles: REACHABILITY_MAX_CYCLES }));
    expect(runs.some((r) => r.attunementCycles["the-deep-beacon-charts"] !== null)).toBe(true);
  });

  it("at least one run reaches AND clears the finale (hollow-crown), honestly gated", { timeout: LONG }, () => {
    const arc = loadArc();
    const runs = REACHABILITY_SEEDS.map((seed) => simulateArcRun(arc, { seed, maxCycles: REACHABILITY_MAX_CYCLES }));
    const clearedFinale = runs.filter((r) => r.clearCycles["hollow-crown"] !== undefined);
