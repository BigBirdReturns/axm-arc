// Wandering Court completion-sim acceptance: exercises the reusable
// cartridge-conformance harness (src/sim/cartridge-conformance.ts) against
// `cartridges/wandering-court.arc.json` — a new importable cartridge, not yet
...
// honesty, reachability/tuning) but through the arc-agnostic helper instead of
// a hand-rolled simulator, adapted for this cartridge's own chapter/gate
// structure (Interlude I / Interlude II / Finale, gated on seals-of-the-season
// rather than sigils-of-the-march).
...
// Pinned content-identity anchor for cartridges/wandering-court.arc.json. If
...
// silently.
const PINNED_DIGEST = "cart1_2e7e1a6b38c1c27f70fd774243fd592a752cb8b9cda6acd04091a4caaea6495c";

const CARTRIDGE_PATH = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)),
  "../../cartridges/wandering-court.arc.json",
);
...
function loadArc(): Arc {
  ...
  if (!imported.ok) throw new Error(`wandering-court.arc.json failed to import: ${imported.errors.join("; ")}`);
...
describe("import", () => {
  it("imports through the real seam (importArcFromJson → validateArc) without errors", () => {
    ...
    if (!result.ok) return;
    expect(result.entry.arc.meta.id).toBe("wandering-court");
...
const CHAPTER_2_CHALLENGES = ["masked-envoy", "tourney-of-claims", "poisoned-banquet"];
const CHAPTER_3_CHALLENGES = ["the-silent-procession", "eclipsed-throne"];
...
describe("gate honesty", () => {
  ...
  it("eclipsed-throne (the finale) is only ever attempted once seals-of-the-season is held by an attuned agent", { timeout: LONG }, () => {
    const arc = loadArc();
    for (const seed of [1, 2, 3, 4, 5, 6, 7, 8]) {
      const run = simulateArcRun(arc, { seed, maxCycles: 45 });
      if (run.attempts["eclipsed-throne"] !== undefined) {
        // The gate is attunementThreshold 0.5 over agentAttunements
        // ["seals-of-the-season"] (grantsAccessTo on the chain) — an attempt
        // can only exist if challengeAccess judged the actual party as
        // satisfying it, and that requires the chain to have completed for at
        // least one party member.
        expect(run.attunementCycles["seals-of-the-season"]).not.toBeNull();
        // The chain's own steps require the-silent-procession cleared, which
        // in turn requires chapter-3 to be unlocked, which requires
        // chapter-2 cleared.
        expect(run.tierClearCycles["chapter-2"]).not.toBeNull();
      }
    }
...
//
// Reviewer's flagged risk: eclipsed-throne is difficultyRating 78 with only a
// 3-5 agent party (champion + confessor required), gated behind the
// seals-of-the-season attunement chain (clear masked-envoy, poisoned-banquet,
// and the-silent-procession — all with the SAME attuned agents, since a chain
...
// bounded seed/cycle sweep, eclipsed-throne clears in the overwhelming
// majority of seeds within 45 cycles, with ZERO engine-reported failures
// anywhere in the run — every non-clear is a "partial" (agent downtime, not
// defeat) that the harness's autoplay policy simply retries. No content
// tuning was needed: the encounter is tight (partial retries before the final
// clear are common) but reachable and clearable as authored. If a future edit
// to the JSON changes this balance, re-run this suite; a swing to
// `stalled`/`max-cycles` dominance here is the signal that tuning
// (thresholds, roster ranges, item bonuses — smallest change that restores
// tight-but-reachable) is warranted, exactly as
...
describe("reachability + tuning (eclipsed-throne finale)", () => {
  it("at least one run clears every chapter-1 and chapter-2 challenge", { timeout: LONG }, () => {
    ...
    for (const id of ["bandit-toll", "village-grievance", "river-crossing-pact", "masked-envoy", "tourney-of-claims", "poisoned-banquet"]) {
      expect(runs.some((r) => r.clearCycles[id] !== undefined)).toBe(true);
    }
  });

  it("at least one run completes the seals-of-the-season attunement chain", { timeout: LONG }, () => {
    const arc = loadArc();
    const runs = REACHABILITY_SEEDS.map((seed) => simulateArcRun(arc, { seed, maxCycles: REACHABILITY_MAX_CYCLES }));
    expect(runs.some((r) => r.attunementCycles["seals-of-the-season"] !== null)).toBe(true);
  });

  it("at least one run reaches AND clears the finale (eclipsed-throne), honestly gated", { timeout: LONG }, () => {
    const arc = loadArc();
    const runs = REACHABILITY_SEEDS.map((seed) => simulateArcRun(arc, { seed, maxCycles: REACHABILITY_MAX_CYCLES }));
    const clearedFinale = runs.filter((r) => r.clearCycles["eclipsed-throne"] !== undefined);
