...
import { ASOIAF_EXTERNAL_PROVENANCE_LINEAGE, asoiafExternalCreditRequired, containsPrivateContact, getAsoiafExternalSource, } from "../../src/narrative/canon/asoiaf/external/index.js";
import { advanceCollectorCursor, cacheKey, collectorEstatePaths, collectorStatus, commitObservation, findCacheEntry, generateCollectorCredits, initializeCollectorEstate, planCollectorWork, readNdjson, recordAttempt, recordGap, sha256, updateSourceLedgerRow, upsertCacheIndex, verifyCollectorEstate, writeJsonAtomic, } from "./asoiaf-external-estate.js";
export const ASOIAF_EXTERNAL_COLLECTOR_USER_AGENT = "axm-arc/asoiaf-external-collector/1 (+https://github.com/BigBirdReturns/axm-arc; bigbirdreturns@proton.me)";
const throttleState = {
    ...
    for (let attempt = 0; attempt <= source.harvestPolicy.retryCount; attempt += 1) {
        ...
        try {
            ...
            if (Number.isFinite(contentLength)
                ...
                throw new CollectorOversizeError(`response declared ${contentLength} bytes; maximum is ${options.maxBytes}`);
            }
            if (!isTransientStatus(response.status) || attempt >= source.harvestPolicy.retryCount) {
                return { response, requestCount };
            }
            const retryAfter = retryAfterMilliseconds(response.headers.get("retry-after"), options.clock.now());
            await options.clock.sleep(retryAfter ?? Math.min(30_000, 1_000 * 2 ** attempt));
        }
        catch (error) {
            if (error instanceof CollectorOversizeError)
                throw error;
            lastError = error instanceof Error ? error : new Error(String(error));
            if (attempt >= source.harvestPolicy.retryCount)
                throw lastError;
...
function robotsAllows(text, target) {
    ...
    const toolToken = "axm-arc";
    const applicable = groups.filter((group) => group.agents.some((agent) => agent === "*" || toolToken.includes(agent)));
...
function networkAccessMethod(source) {
...
function normalizeTextBody(bytes, mediaType) {
...
function responseTitle(source, response) {
    return response.headers.get("x-title")?.trim() || source.label;
}
function automaticCredit(source, title, sourceUri) {
    if (!asoiafExternalCreditRequired(source))
        return null;
    return {
        title,
        author: source.label,
...
function relativeCacheUri(root, absolute) {
...
async function responseBytes(response, maximum) {
    ...
    if (bytes.byteLength > maximum) {
        throw new CollectorOversizeError(`response contained ${bytes.byteLength} bytes; maximum is ${maximum}`);
    }
    return bytes;
}
...
function observationFromCache(root, observationId) {
    const paths = initializeCollectorEstate(root);
    return (readNdjson(paths.observations).find((observation) => observation.observationId === observationId) ?? null);
}
...
export async function collectNetworkSource(options) {
    ...
    try {
        ...
        const bytes = await responseBytes(fetched.response, source.harvestPolicy.maxResponseBytes);
        const digest = contentDigest(bytes);
        const mediaType = (fetched.response.headers.get("content-type") ?? "application/octet-stream")
            .split(";", 1)[0]
            .trim()
            .toLowerCase();
        const normalizedText = normalizeTextBody(bytes, mediaType);
        let retention = "metadata-only";
        let excerpt = null;
        let cacheUri = null;
        let retainedValue = undefined;
        if (source.harvestPolicy.retainRawBody) {
            if (!["cc0", "cc-by", "cc-by-sa", "public-domain"].includes(source.rightsMode)) {
                const terminal = recordGap({
                    root: options.root,
                    sourceId: source.id,
                    sourceRecordId,
        ...
        const committed = commitObservation({
            ...
            credit: automaticCredit(source, title, fetched.response.url || target.href),
            retainedValue,
            requestCount,
            cacheHit: false,
        });
...
async function streamFileDigest(filePath) {
...
export async function collectLocalSource(options) {
    const source = getAsoiafExternalSource(options.sourceId);
    if (!source)
        throw new Error(`unknown source ${options.sourceId}`);
    if (source.harvestPolicy.mode !== "local-private-only") {
    ...
    const existing = findCacheEntry(options.root, source.id, options.sourceRecordId);
    if (existing
...
export async function collectPlannedSources(input) {
    ...
    for (const row of planned) {
        ...
        results.push(await collectNetworkSource({
            root: input.root,
            sourceId: source.id,
            refresh: input.refresh,
            fetchImpl: input.fetchImpl,
...
export function collectorPublicStatus(root) {
    return collectorStatus(root);
}
export function collectorVerify(root) {
    return verifyCollectorEstate(root);
}
export function collectorInitialize(root, now) {
...
export function collectorCredits(root) {
    return generateCollectorCredits(root);
}
export function writeCollectorReceiptSummary(root, outputPath) {
