...
import { collectorContentId, collectorEstatePaths, readJson, readNdjson, sha256, writeJsonAtomic, } from "./asoiaf-external-estate.js";
export const ASOIAF_LOCAL_EDITION_MANIFEST_FORMAT = "axm-asoiaf-local-edition-manifest/1";
export const ASOIAF_LOCAL_EDITION_UNIT_FORMAT = "axm-asoiaf-local-edition-unit/1";
export const ASOIAF_LOCAL_EDITION_SEGMENT_FORMAT = "axm-asoiaf-local-edition-segment/1";
export const ASOIAF_LOCAL_EDITION_PRIVATE_INDEX_FORMAT = "axm-asoiaf-local-edition-private-index/1";
export const ASOIAF_LOCAL_EDITION_RECEIPT_FORMAT = "axm-asoiaf-local-edition-receipt/1";
...
function editionKey(sourceId, editionId) {
...
export function localEditionPaths(root, sourceId, editionId) {
...
function writeNdjsonAtomic(filePath, values) {
...
function parseZipDirectory(bytes, maxEntries, maxExpandedBytes) {
    ...
    for (let index = 0; index < totalEntries; index += 1) {
        ...
        if (end > bytes.length)
            throw new Error(`ZIP central directory entry ${index} exceeds source bounds`);
        const name = cleanZipName(bytes.subarray(offset + 46, offset + 46 + nameLength).toString("utf8"));
        if (flags & 0x1)
            throw new Error(`encrypted ZIP member ${name} is not supported`);
...
function materializeZipEntry(source, entry) {
    ...
    if (dataEnd > source.length)
        throw new Error(`ZIP member ${entry.name} exceeds source bounds`);
    ...
    if (expanded.length !== entry.uncompressedSize) {
        throw new Error(`ZIP member ${entry.name} expanded to ${expanded.length} bytes; expected ${entry.uncompressedSize}`);
    }
    return { ...entry, bytes: expanded };
}
function resolvePackagePath(packagePath, href) {
    const base = path.posix.dirname(packagePath);
    return cleanZipName(path.posix.join(base, href.split("#", 1)[0] ?? href));
}
function navLabels(packagePath, items, entries) {
    const output = new Map();
    const nav = [...items.values()].find((item) => (item.properties ?? "").split(/\s+/).includes("nav"));
    if (!nav?.href)
        return output;
    ...
    for (const match of html.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a\s*>/gi)) {
        ...
        if (label && !output.has(target))
            output.set(target, label);
    }
    return output;
}
function parseEpub(source, maxEntries, maxExpandedBytes) {
    ...
    if (!packageEntry)
        throw new Error(`EPUB package document ${packagePath} is missing`);
    ...
    for (const match of opf.matchAll(/<item\b([^>]*?)(?:\/?>)/gi)) {
        ...
        if (attributes.id && attributes.href)
            items.set(attributes.id, attributes);
    }
    ...
    if (units.length === 0)
        throw new Error("EPUB package contains no readable spine documents");
    return {
        inputFormat: "epub",
...
function parsePlainText(source) {
    ...
    if (!text)
        throw new Error("plain-text source is empty");
    ...
    if (headings.length >= 2) {
        ...
        for (let index = 0; index < headings.length; index += 1) {
            ...
            const unitText = normalizeText(lines.slice(current.index + 1, next?.index ?? lines.length).join("\n"));
            if (!unitText)
                continue;
            units.push({
                label: current.line.trim(),
                sourcePart: `plain-text:heading-${current.index}`,
                sourcePartDigest: digestText(lines.slice(current.index, next?.index ?? lines.length).join("\n")),
                text: unitText,
            });
        }
    }
...
function parseHtml(source) {
    ...
    const text = normalizeText(stripMarkup(markup));
    if (!text)
        throw new Error("HTML source contains no readable text");
...
function detectAndParse(source, options) {
...
function privateTextFileName(order, unitId) {
...
function writePrivateText(input) {
    ...
    const entries = input.units.map((unit, index) => {
        ...
        if (text === undefined)
            throw new Error(`missing private text for ${unit.unitId}`);
...
export async function intakeAsoiafLocalEdition(options) {
    ...
    if (!stats.isFile())
        throw new Error("local edition input is not a regular file");
...
export function readAsoiafLocalEditionManifest(root, sourceId, editionId) {
    const paths = localEditionPaths(root, sourceId, editionId);
    if (!fs.existsSync(paths.manifest)) {
        throw new Error(`local edition ${editionId} is not initialized`);
    }
    return readJson(paths.manifest, null);
}
function safeEditionRelative(editionRoot, relativeUri) {
...
export function verifyAsoiafLocalEdition(root, sourceId, editionId) {
    const errors = [];
    const paths = localEditionPaths(root, sourceId, editionId);
    if (!fs.existsSync(paths.manifest))
        return ["missing edition manifest"];
    ...
    if (manifest.unitCount !== units.length)
        errors.push("unit count mismatch");
    if (manifest.segmentCount !== segments.length)
    ...
    for (const unit of units) {
        const unitSegments = (segmentsByUnit.get(unit.unitId) ?? []).sort((left, right) => left.paragraphIndex - right.paragraphIndex);
        if (unit.paragraphCount !== unitSegments.length) {
            errors.push(`unit ${unit.unitId} paragraph count mismatch`);
        }
        if (JSON.stringify(unit.segmentIds)
    ...
    if (manifest.privateTextRetained) {
        if (!fs.existsSync(paths.privateIndex)) {
            ...
            const index = readJson(paths.privateIndex, null);
            ...
            for (const entry of index.entries) {
                ...
                if (digestText(text) !== entry.textDigest
                ...
                for (const segment of segmentsByUnit.get(entry.unitId) ?? []) {
                    ...
                    if (digestText(value) !== segment.textDigest
    ...
    const observations = readNdjson(collectorPaths.observations);
    if (!manifest.collectorObservationId
        || !observations.some((observation) => observation.observationId === manifest.collectorObservationId
...
export function writeAsoiafLocalEditionReceipt(root, sourceId, editionId, verifiedAt = new Date().toISOString(), outputPath) {
    const paths = localEditionPaths(root, sourceId, editionId);
...
export function listAsoiafLocalEditions(root) {
    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        ...
        if (fs.existsSync(target)) {
            manifests.push(readJson(target, null));
        }
    }
...
export function locateAsoiafLocalEditionText(input) {
    ...
    const manifest = readAsoiafLocalEditionManifest(input.root, input.sourceId, input.editionId);
    if (!manifest.privateTextRetained)
        throw new Error("edition has no retained private text");
    const units = readNdjson(paths.units);
    const segments = readNdjson(paths.segments);
    const unit = units.find((entry) => entry.unitId === input.unitId);
    if (!unit)
        throw new Error(`unknown local-edition unit ${input.unitId}`);
    const segment = input.segmentId
        ? segments.find((entry) => entry.segmentId === input.segmentId && entry.unitId === unit.unitId) ?? null
        : null;
    if (input.segmentId && !segment)
        throw new Error(`unknown unit segment ${input.segmentId}`);
    ...
    if (!privateEntry)
        throw new Error(`private text index lost ${unit.unitId}`);
    const target = safeEditionRelative(paths.editionRoot, privateEntry.relativeUri);
    if (!target || !fs.existsSync(target))
        throw new Error(`private text for ${unit.unitId} is unavailable`);
